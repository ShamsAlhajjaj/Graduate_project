...
mySpeech_en.speak(
    "To control the book player, use the following commands: play, or on, or resume: to play the book. stop, or of, or pause: to stop the book. Next: To play the next book. Previous: To play the previous book. Up, or raise: to raise the volume. down, or low: to lower the volume"
);
...
function parseResult() {
    ...
    if (mostrecentword.indexOf("play") !== -1 ||
        ...
        mostrecentword.indexOf("صامت") !== -1) {
        muteUnmute()
    } else if (mostrecentword.indexOf("random") !== -1 ||
        mostrecentword.indexOf("shuffle") !== -1) {
        randomTrack()
...
function dataObj($img, $name, $artist, $book)
{
...
foreach ($query as $data) {
    ...
    //echo "<h1>" . "<br>" . $img . "<br>" . $name . "<br>" . $artist . "<br>" . $book . "</h1>";
    $pushScript = "<script> book_list.push(" . dataObj($img, $name, $artist, $book) . "); </script>";
    echo $pushScript;
}
