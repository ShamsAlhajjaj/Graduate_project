 <div class="Side_Body">
     <div id="SBMainFn" class="SideBar">
         <button class="SBBtns" onclick="visitPage('index.php')" onmouseover="readText(this.innerText)"
             onmouseleave="stopSpeak()">Home Page</button>
         <button class="SBBtns" onclick="visitPage('categories.php')" onmouseover="readText(this.innerText)"
             onmouseleave="stopSpeak()">Categories</button>
         <button class="SBBtns" onclick="visitPage('advance_search.php')" onmouseover="readText(this.innerText)"
             onmouseleave="stopSpeak()">Advanced Search</button>
         <button class="SBBtns" onclick="visitPage('request_book.php')" onmouseover="readText(this.innerText)"
             onmouseleave="stopSpeak()">Request a Book</button>
         <button class="SBBtns" onclick="visitPage('login_volunteer.php')" onmouseover="readText(this.innerText)"
             onmouseleave="stopSpeak()">Join Us As a Volunteer</button>
         <button class="SBBtns" onclick="visitPage('about_us.php')" onmouseover="readText(this.innerText)"
             onmouseleave="stopSpeak()">About Us</button>
         <button class="SBBtns" onclick="visitPage('help.php')" onmouseover="readText(this.innerText)"
             onmouseleave="stopSpeak()">Help</button>
         <div class="hola"></div>
     </div>
     <button id="SBBtnFn" class="SBButton" onclick="SHButton()">
         <img id="SBArrwIcn" src="_images/side-bar-icons/close_button_icon.png" width="18px"></button>
 </div>